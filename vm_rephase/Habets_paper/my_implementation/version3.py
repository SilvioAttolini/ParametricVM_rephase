import numpy as np
from scipy.signal import stft, bessel
from matplotlib import pyplot as plt
from scipy.signal.windows import hann
from scipy.special import sinc, jv
from mix_signals3 import mix_signals3


def version3():
    """ """

    # Initialization
    Fs = 8000  # Sample frequency (Hz)
    c = 340  # Sound velocity (m/s)
    K = 256  # FFT length
    M = 2  # Number of sensors
    d = 0.2  # Inter sensor distance (m)
    type_nf = 'spherical'  # Type of noise field: 'spherical' or 'cylindrical'
    L = 10 * Fs  # Data length

    # Generate M mutually independent input signals of length L
    np.random.seed(1)
    n = np.random.randn(L, M)

    # Generate matrix with desired spatial coherence
    ww = 2 * np.pi * Fs * (np.arange(K // 2 + 1)) / K
    DC = np.zeros((M, M, K // 2 + 1))
    for p in range(M):
        for q in range(M):
            if p == q:
                DC[p, q, :] = np.ones(K // 2 + 1)
            else:
                switch_type = type_nf.lower()
                if switch_type == 'spherical':
                    DC[p, q, :] = sinc(ww * abs(p - q) * d / (c * np.pi))

                elif switch_type == 'cylindrical':
                    DC[p, q, :] = bessel(0, ww * abs(p - q) * d / c)

                else:
                    raise ValueError('Unknown noise field.')

    # Generate sensor signals with desired spatial coherence
    # x = mix_signals(n, DC, 'cholesky')  # Assuming mix_signals is a custom function
    x = mix_signals3(n, DC, 'eigen')  # Assuming mix_signals is a custom function

    # Compare desired and generated coherence
    K_eval = 256
    ww = 2 * np.pi * Fs * (np.arange(K_eval // 2 + 1)) / K_eval
    sc_theory = np.zeros((M - 1, K // 2 + 1))
    sc_generated = np.zeros((M - 1, K // 2 + 1))

    # Calculate STFT and PSD of all output signals
    X = stft(x, window=hann(K_eval), noverlap=int(0.75 * K_eval), nperseg=K_eval)  #, centered=False)
    X = X[:K_eval // 2 + 1, :, :]
    phi_x = np.mean(np.abs(X) ** 2, axis=2)

    # Calculate spatial coherence of desired and generated signals
    for m in range(M - 1):
        switch_type = type_nf.lower()
        if switch_type == 'spherical':
            sc_theory[m, :] = sinc(ww * m * d / (c * np.pi))
        elif switch_type == 'cylindrical':
            sc_theory[m, :] = bessel(0, ww * m * d / c)

        # Compute cross-PSD of x_1 and x_(m+1)
        psi_x = np.mean(X[:, :, 1] * np.conj(X[:, :, m + 1]), axis=2)

        # Compute real-part of complex coherence between x_1 and x_(m+1)
        sc_generated[m, :] = np.real(psi_x / np.sqrt(phi_x[:, 0, 0] * phi_x[:, 0, m + 1])).T

    # Calculate normalized mean square error
    NMSE = np.zeros(M)
    for m in range(M - 1):
        NMSE[m] = 10 * np.log10(np.sum(((sc_theory[m, :] - sc_generated[m, :]) ** 2)) / np.sum(sc_theory[m, :] ** 2))

    # Plot spatial coherence of two sensor pairs
    plt.figure(1)
    MM = min(2, M - 1)
    Freqs = np.arange(0, Fs / 2, Fs / (2 * K)) / 1000
    for m in range(MM):
        plt.subplot(MM, 1, m)
        plt.plot(Freqs, sc_theory[m, :], '-k', linewidth=1.5)
        plt.plot(Freqs, sc_generated[m, :], '-.b', linewidth=1.5)
        plt.xlabel('Frequency [kHz]')
        plt.ylabel('Real(Spatial Coherence)')
        plt.title(f'Inter sensor distance {m * d:.2f} m')
        plt.legend(['Theory', f'Proposed Method (NMSE = {NMSE[m]:.1f} dB)'])
        plt.grid(True)

    return
